...
from mutopia.utils import logger, stream_subprocess_output
...
class QueryRecord:
    ...
    @classmethod
    def get_format_str(cls, weight_col=None, cluster=True):
        return (
...
def _extract_mutation_info(
    chrom: str, pos: int, ref: str, alt: str, check_ref=True, *, fasta_object
):
    ...
    try:
        ...
        raise KeyError(
            "\tChromosome {} found in VCF file is not in the FASTA reference file".format(
                chrom
            )
        ) from err
    ...
    if not ref.upper() in "ATCG" or not alt.upper() in "ATCG":
        raise WeirdMutationError(
            "\tWeird mutation found at {}:{} {}->{}".format(chrom, str(pos), ref, alt)
        )
...
def _process_query_line(line, fasta_object):
    ...
    try:
        ...
        raise WeirdMutationError(f"Could not parse the following line: {line}") from err

    try:
        query.WEIGHT = min(float(query.WEIGHT), 1.0)
    except ValueError as err:
        raise BadWeightError(
            f"Could not parse the following weight: {query.WEIGHT}"
        ) from err

    (context, alt, configuration_idx) = _extract_mutation_info(
        query.CHROM, query.POS0, query.REF, query.ALT, fasta_object=fasta_object
    )
    ...
    adjusted_weight = (
        query.WEIGHT
        if (query.CLUSTER_SIZE <= 3)
        else (query.WEIGHT / query.CLUSTER_SIZE)
    )
...
def featurize_mutations(
    ...
    if weight_col is None and sample_name is None:
        num_samples = len(
            subprocess.check_output(f"bcftools query -l {vcf_file}", shell=True)
            .decode()
            .strip()
            .split("\n")
        )
        assert (
            num_samples <= 1
        ), "Multiple samples were found in this vcf file. You must specify a `sample_name` in order to extract mutation weights."
    ...
    with tempfile.NamedTemporaryFile("w") as processed_vcf:
        ...
        query_fn = partial(
            query_fn,
            input_vcf,
            QueryRecord.get_format_str(weight_col=weight_col, cluster=cluster),
    ...
    logger.info(
        f"Successfully ingested {n_ingested} mutations, {n_weird} mutations could not be parsed."
    )
