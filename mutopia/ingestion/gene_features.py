...
def make_annotation(gtf, output):
    subprocess.check_call(
        " ".join([
            ...
            "-attr", "gene_id",
            "--header",
            "-f", r'"{chrom}\t{start}\t{end}\t{attributes[gene_name]}\t{attributes[gene_id]}\t{strand}\n"',
        ]),
        shell=True,
    )
...
def join_quantitation(
    ...
    *quantitation_files,
    ...
    for file in quantitation_files:
        df = pd.read_csv(file, sep=None, header=None, engine="python", names=[join_on, "expression_est"])
        quantifications.append(df)

    quantifications = (
        pd.concat(quantifications, axis=0)
        .groupby(join_on)
        .mean()
        .reset_index()
    )
    ...
    if shared_count == 0:
        raise ValueError(f"No matching {join_on} values found between gene annotation and quantification files.")
