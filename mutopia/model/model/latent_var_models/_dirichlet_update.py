...
def dirichlet_multinomial_logprob(z, alpha):
    ...
    return (
        ...
        + gammaln(n + 1)
        - gammaln(n + alpha_bar)
        + np.sum(gammaln(n_z + alpha) - gammaln(alpha) - gammaln(n_z + 1))
    )
...
def dirichlet_bound(alpha, gamma):
    ...
    return (
        ...
        + np.sum(
            gammaln(gamma) - gammaln(alpha) + (alpha - gamma) * logE_gamma, axis=-1
        )
    )
...
def _dir_prior_objective(alpha, N, logphat):
    return -N * (
        gammaln(np.sum(alpha)) - np.sum(gammaln(alpha)) + np.sum((alpha - 1) * logphat)
    )
...
def _dir_prior_update_step(prior, N, logphat):
    ...
    if np.any(new_prior < 0.00001):
        logger.debug("Performing projected gradient descent update.")
        new_prior = np.maximum(new_prior, 0.00001)

    return new_prior


def _dir_prior_newton_iter(prior, N, logphat, compare_prior=None, max_iter=100):
    ...
    logger.debug(
        f'Prior log-likelihood improvement: {curr_val - new_val}\nNew prior: {" ".join(map(str,prior))}'
    )
    return prior
