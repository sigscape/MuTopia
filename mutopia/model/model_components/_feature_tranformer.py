...
def get_feature_interaction_group_idxs(
    ...
    groups = [
        CS.get_features(corpus)[feature_name.rsplit(":", 1)[0]].attrs["group"]
    ...
    interacting_features = [
        f"\n\t- {k}: {str_wrapped_list(v).strip()}"
        for k, v in interacting_features.items()
    ]
...
def get_categorical_feature_idxs(
    ...
    output_slice: slice = transformer["split_transformer"].output_indices_[
        "categorical"
    ]
...
def get_known_categories(
    ...
    cat_encoder = transformer["split_transformer"].named_transformers_["categorical"]
...
def get_shared_features(*corpus_states):
    ...
    shared_features = reduce(
        ...
        (
            set(
                ...
                for fname, feature in CS.get_features(state).items()
                if not FeatureType(feature.attrs["normalization"])
                in (FeatureType.MESOSCALE, FeatureType.STRAND)
            )
    ...
    for fname, ftypes in seen_types.items():
        if len(ftypes) > 1:
            raise ValueError(
                f"Feature {fname} has multiple normalizations: {str_wrapped_list(ftypes)}"
            )
    ...
    return list(
        zip(
            *sorted(
                shared_features,
                key=lambda x: (
                    not FeatureType(x[1]) == FeatureType.CATEGORICAL,
                    x[0],
...
class PasteTransformer(BaseEstimator, TransformerMixin, OneToOneFeatureMixin):
    ...
    def get_feature_names_out(self, input_features):
        ...
        if not self.n_features_in_ == len(input_features):
            raise ValueError(
                "The number of input features must match the number of columns in the input data!"
            )
...
def get_paste_transformer(feature_names, add_corpus_intercepts=False):
    return (
        PasteTransformer(
            feature_names=feature_names, add_corpus_intercepts=add_corpus_intercepts
        )
...
class Convolve(BaseEstimator, TransformerMixin, OneToOneFeatureMixin):
    ...
    def transform(self, X, y=None, *, corpus):
        ...
        def uncollapse(idxs, group_sizes):
            return list(
                chain.from_iterable(repeat(i, n) for i, n in zip(idxs, group_sizes))
            )

        def r1_gt_r2(r1, r2):
            (r1_chrom, r1_start, r1_end) = r1[:3]
            (r2_chrom, r2_start, r2_end) = r2[:3]
            return r1_chrom > r2_chrom or (r1_chrom == r2_chrom and r1_start >= r2_end)

        regions = CS.get_regions(corpus)
        data = enumerate(
            zip(
                regions.chrom.data,
                regions.start.data,
        ...
        data = streaming_groupby(
            ...
            has_lapsed=lambda curr, buffer: all(
                r1_gt_r2(curr[1], b[1]) for b in buffer
            ),
        )
        ...
        X = np.hstack([X[uncollapse(offset, group_sizes)] for offset in offsets])
    ...
    def get_feature_names_out(self, input_features):
        ...
        for w in range(-self.width, self.width + 1):
            feature_names.extend(
                [f"{f}:{w}" if w <= 0 else f"{f}:+{w}" for f in input_features]
            )
...
def get_categorical_transformer(
    ...
    *corpuses,
    ...
    categories_lists = [
        get_feature_classes(example_corpus, feature) for feature in feature_names_in
    ]

    if add_corpus_intercepts:
        categories_lists = [
            [CS.get_name(corpus) for corpus in corpuses]
        ] + categories_lists
...
def get_normalizing_transformer(
    ...
    *corpus_states,
    ...
    if any(
        f in (FeatureType.CATEGORICAL, FeatureType.MESOSCALE) for f in feature_types
    ):
    ...
    return ColumnTransformer(
        [
            ...
            (
                ...
                type_dict[FeatureType.QUANTILE],
            ),
...
def get_feature_transformer(
    *corpuses,
    ...
    return (pipeline_, feature_names_in)
...
class StratifiedTransformer:
    ...
    def transform(self, X, **kwargs):
        try:
            ...
            transformer = clone(self.base_transformer).fit(X, **kwargs)
