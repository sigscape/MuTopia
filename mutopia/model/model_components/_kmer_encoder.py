...
def _get_subsequence_coding(
    ...
    def get_n_outcomes(pos):
        return reduce(lambda x, y: x * y, (len(alphabet_by_position[j]) for j in pos))
...
def _encode_subsequences(alphabet_by_position, subsequence_coding, observation):
...
def _compose_subsequence_encoders(
    ...
    for l in range(1, observation_len + 1):
        ...
        _n_coefs, subsequence_coding = _get_subsequence_coding(alphabet_by_position, l)
        ...
        subsequence_encoders.append(partial(_offset_encoding, encoder, n_coefs))

        n_coefs += _n_coefs

    return (partial(_cat_encoding, subsequence_encoders), n_coefs)
...
class KmerEncoder(ContextEncoder):
    ...
    def fit(self, context_names):
        ...
        self.encoder_, self._n_coefs = _compose_subsequence_encoders(
            self.alphabet_by_position
        )
        return self

    def get_feature_names_out(self):
        return _compose_feature_names(self.feature_name_fn, self.alphabet_by_position)
    ...
    def encoding_matrix(self):
        ...
        i, j = [], []
        for row, v in enumerate(map(self.kmer_extractor, self.context_names_)):
