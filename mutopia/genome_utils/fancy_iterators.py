    ...
    for curr in iter:
        ...
        try:
            ...
            if not key(curr) >= key(_prev):
                raise ValueError(
                    "Items are out of order, {} should be greater than {}".format(
                        str(key(curr)), str(key(_prev))
                    )
                )
...
def interleave_streams(*iterators, key=lambda x: x):
    ...
    streams = [PeekIterator(sorted_iterator(stream, key=key)) for stream in iterators]
