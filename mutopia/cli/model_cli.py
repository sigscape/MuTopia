...
def report(
    ...
    """
    try:
        output_path = model_core.generate_model_report(model_path, output)
        click.echo(f"Model report generated: {output_path}")
    except Exception as e:
...
@click.option(
    ...
    type=click.IntRange(1, 1000),
    default=1,
    help="Number of parallel threads for prediction (more threads = faster processing)",
)
@click.option(
    "--contributions/--no-contributions",
    is_flag=True,
    default=True,
    help="Calculate detailed component contributions per genomic locus",
)
@click.option(
    "--output",
    "-o",
    type=click.Path(writable=True),
    default=None,
    help="Output file for predictions (default: modify input dataset in-place)",
)
def predict(
    model: str,
    dataset: str,
    ...
    """
    try:
        result_path = model_core.predict_model(
            model_path=model,
        ...
        if result_path:
            click.echo(f"Predictions saved to: {result_path}")
        else:
            click.echo("Dataset modified in-place with predictions")
    except Exception as e:
        raise click.ClickException(f"Prediction failed: {str(e)}")


@model.command("SHAP", short_help="Calculate SHAP values for model interpretability")
@click.argument("model", type=click.Path(exists=True), metavar="MODEL_FILE")
@click.argument("dataset", type=click.Path(exists=True), metavar="DATASET_FILE")
@click.argument("components", type=str, nargs=-1, metavar="COMPONENT...")
@click.option(
    "-@",
    "--threads",
    type=click.IntRange(1, 1000),
    default=1,
    help="Number of parallel threads for SHAP calculation (more threads = faster processing)",
)
@click.option(
    "--n-samples",
    type=click.IntRange(1, 100000),
    default=2000,
    help="Number of background samples for SHAP value estimation (more = more accurate)",
)
@click.option(
    "--scan/--no-scan",
    type=bool,
    default=False,
    is_flag=True,
    help="Use scanning approach for SHAP calculation (slower but more memory efficient)",
)
def shap(
    model: str,
    dataset: str,
    ...
    """
    try:
        model_core.calculate_shap_values(
            model_path=model,
            dataset_path=dataset,
        ...
        click.echo("SHAP values calculated and saved to dataset")
    except Exception as e:
        raise click.ClickException(f"SHAP calculation failed: {str(e)}")
...
@click.argument("model", type=click.Path(exists=True), metavar="MODEL_FILE")
@click.argument("dataset", type=click.Path(exists=True), metavar="DATASET_FILE")
@click.argument("output", type=click.Path(writable=True), metavar="OUTPUT_EXCEL")
def excel(
    model: str,
    dataset: str,
    output: str,
    ...
    """
    try:
        model_core.export_to_excel(
            model_path=model,
            dataset_path=dataset,
            output_path=output,
        )
        click.echo(f"Excel report exported to: {output}")
    except Exception as e:
        raise click.ClickException(f"Excel export failed: {str(e)}")
